        ...
        <>
                ...
                <div className="mt-8 grid grid-cols-12 gap-4">
                    ...
                    flex gap-x-4 items-center justify-between">
                        <div className="flex-y-center gap-4">
                            <h1>ช่องทางติดต่อ</h1>
                            <Link href={'/'} className="text-xl text-blue-500"><FaFacebookSquare/></Link>
                            <Link href={'/'} className="text-xl text-green-500"><FaLine/></Link>
                        </div>
                        <p className="text-lg flex-y-center">
                            <PiPhoneCallFill className="hidden sm:block"/>
                            <span className="ms-1 mt-1 text-sm opacity-70 font-medium">0904445814</span>
                        </p>
            ...
            <div className="fixed bottom-4 right-4 flex flex-col items-center gap-4 z-999">
                { status === 'authenticated' &&
                    <button onClick={()=>dormitoryOnlyStore.addFavorite(Number(dmtId), session?.user?.id)}
                ...
                ${open && 'outline outline-offset-2 outline-blue-400'}`}>
                    <RxDashboard/>
                </button>
            </div>
        </>
